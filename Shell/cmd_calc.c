...
#ifdef __linux__
static void* subthread(void* none);

void cmd_calc(char** input) {
	if (!strcmp(input[1], "calc")) {
		int idx = 0, err = 0;
		idx = snprintf(input[0], CLSIZE, "/bin/gnome-calculator");
		...
		if (param == NULL) system(input[0]);
			...
			if (!strcmp(param, "&") && next == NULL) {
				pthread_t thread = 0;
				pthread_create(&thread, NULL, subthread, NULL);
			}
			else printf("Incorrect command syntax, do you mean \"calc &\"?\n\n");
		}
		memset(input[0], 0, idx);
